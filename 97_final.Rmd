...
raw_data <- readxl::read_xlsx("raw-data/country_Set.xlsx") %>% 
  clean_names() %>%
  rename(country = country_name) # for joining purposes

pop <- readxl::read_xlsx("raw-data/WPP2019_POP_F01_1_TOTAL_POPULATION_BOTH_SEXES.xlsx") %>%
  clean_names()
```
...
regions <- readxl::read_xlsx("raw-data/regions.xlsx") %>%
  clean_names() %>%
  rename(country = name) %>%
